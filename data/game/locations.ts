import {
  questHelper,
  annotateQuestChoices,
  type QuestChoice,
  type QuestSkillRequirement,
} from "./questHelper.js";
...
type QuestProficiencyMatch = QuestSkillRequirement['proficiencies'][number];

function flattenQuestText(value: unknown): string[] {
...
function questAnalysisText(quest: Quest): string {
  return [
    quest.title,
    quest.description,
    quest.timeline,
...
function inferQuestHabitat(quest: Quest): Habitat {
...
function collectQuestProficiencies(quest: Quest): QuestProficiencyMatch[] {
  const matches = new Map<string, QuestProficiencyMatch>();
  (quest.skillRequirements || []).forEach((requirement) => {
    requirement.proficiencies.forEach((match) => {
      ...
      if (!existing) {
        ...
        matches.set(match.id, { ...existing, minimum: match.minimum });
      }
    });
  });
...
function generateQuestChoices(quest: Quest, existing: QuestChoice[] = []): QuestChoice[] {
  const generated: QuestChoice[] = [];
...
function detectSeasonPreferences(text: string): CalendarSeason[] {
  const seasons = new Set<CalendarSeason>();
  if (/(winter|snow|frost|ice|midwinter|yule|solstice)/.test(text)) seasons.add('Winter');
  if (/(spring|thaw|bloom|seedling|planting|sprout)/.test(text)) seasons.add('Spring');
  if (/(summer|heat|sun-baked|solstice|dry spell)/.test(text)) seasons.add('Summer');
  if (/(autumn|fall|harvest|reaping|leaf-fall)/.test(text)) seasons.add('Autumn');
...
function createDefaultQuestVisibility(quest: Quest): QuestVisibilityRule {
  const text = questAnalysisText(quest);
  ...
  return ({ weather, date, random }) => {
    ...
    if (quest.highPriority) demand += 0.15;
    if (requiresRain && (weather.condition === 'rain' || weather.storm)) demand += 0.18;
    if (avoidStorm && weather.condition === 'clear' && !weather.storm) demand += 0.1;
    if (weather.floodRisk === 'warning' && !requiresRain) demand -= 0.12;
    if (weather.droughtStage === 'warning' && requiresRain) demand -= 0.08;
    if (mentionsHarvest && weather.season === 'Autumn') demand += 0.12;
    if (mentionsHarvest && weather.season !== 'Autumn') demand -= 0.08;
    if (prefersCold && weather.temperatureC > 25) demand -= 0.08;
    if (prefersHeat && weather.temperatureC < 5) demand -= 0.08;
    if (cycle === 'weekly') {
...
function extractPrimaryLocation(location: Quest['location']): string | undefined {
...
function ensureQuestVisibilityBinding(quest: Quest): QuestVisibilityBinding {
  const binding: QuestVisibilityBinding = quest.visibilityBinding
    ? { ...quest.visibilityBinding }
    : { region: 'waves_break', habitat: 'urban' };
  if (!binding.region) binding.region = 'waves_break';
  if (!binding.habitat) binding.habitat = inferQuestHabitat(quest);
  if (!binding.location) {
    const derived = extractPrimaryLocation(quest.location);
    if (derived) binding.location = derived;
  }
...
function createQuest(
  ...
  const quest = questHelper({ title, description, ...opts }) as Quest;
