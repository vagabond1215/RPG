...
const DEFAULT_SCHOOL_TIERS: Record<SchoolId, Tier> = {
  destruction: "MINOR",
  enhancement: "MINOR",
  enfeeblement: "MINOR",
  control: "MINOR",
...
const DEFAULT_SPELL_PROFICIENCY = 1;

const DEFAULT_SCALER_OPTIONS: Parameters<SpellScaler["scale"]>[2] = {
  usePrefix: false,
  useSuffix: false,
};
...
const buildDefaultSpellsForElement = (element: ElementId): SpellEntry[] => {
  return ALL_SCHOOLS.map(school =>
    postProcessSpell(
      makeSpell({
        element,
        school,
        tier: DEFAULT_SCHOOL_TIERS[school],
        proficiency: DEFAULT_SPELL_PROFICIENCY,
        scalerOpts: DEFAULT_SCALER_OPTIONS,
      }),
    ),
  );
};
...
const buildElementSpellbook = (element: ElementId, total = 18, rngSeed = 0x13572468): SpellEntry[] => {
  ...
  const defaults = buildDefaultSpellsForElement(element);
  const remaining = Math.max(total - defaults.length, 0);
  const plan = remaining > 0 ? generatePlanForElement(element, remaining, rng) : [];
  const withSpecials = remaining > 0 ? applySpecials(element, plan) : [];
  const generated = withSpecials.map(spec =>
    postProcessSpell(
      makeSpell({
